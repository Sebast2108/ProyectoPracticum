...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ route('medico.create') }}" class="btn btn-success">
            <i class="bi bi-plus-circle"></i> Crear nuevo médico
        </a>
    </div>

    {{-- Barra de búsqueda para filtrar médicos --}}
    <div class="mb-3">
        <input type="text" id="searchInput" class="form-control" placeholder="Buscar médicos por nombre, apellido, especialidad...">
    </div>

    {{-- Tabla responsiva --}}
    <div class="table-responsive">
        <table class="table table-hover align-middle" id="medicosTable" data-sort-dir="asc">
            <thead class="table-dark">
                <tr>
                    ...
                    <th scope="col" style="cursor:pointer" onclick="sortTable(2)">ID Médico <i class="bi bi-arrow-down-up"></i></th>
                    <th scope="col" style="cursor:pointer" onclick="sortTable(3)">Correo <i class="bi bi-arrow-down-up"></i></th>
                    <th scope="col" style="cursor:pointer" onclick="sortTable(4)">Especialidad <i class="bi bi-arrow-down-up"></i></th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        {{-- Botón: Ver --}}
                        <a href="{{ route('medico.show', $m->id) }}" class="btn btn-info btn-sm" title="Ver">
                            <i class="bi bi-eye"></i>
                        </a>

                        {{-- Botón: Editar --}}
                        <a href="{{ route('medico.edit', $m->id) }}" class="btn btn-warning btn-sm" title="Editar">
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('keyup', function() {
        ...
        const rows = document.querySelectorAll('#medicosTable tbody tr');
        rows.forEach(row => {
    ...
    confirmDeleteModal.addEventListener('show.bs.modal', function (event) {
        ...
        var medicoId = button.getAttribute('data-medicoid');
        var form = document.getElementById('deleteForm');
        form.action = '/medico/' + medicoId;
    });
</script>
@endsection
